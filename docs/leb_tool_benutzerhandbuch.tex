...
\begin{itemize}
  ...
  \item HTTPS-fähiges Hosting (empfohlen)
...
\begin{enumerate}
  ...
  \item \textbf{Applikationspfade setzen:} \texttt{base\_path} (relativer Pfad zur Installation) und \texttt{public\_base\_url} (vollständige URL) prüfen.
  ...
  \item \textbf{Admin-Account anlegen:} Zugangsdaten im Installer vergeben.
  \item \textbf{Abschluss und Absicherung:} Installation abschließen und \texttt{install.php} anschließend löschen oder umbenennen.
  \item \textbf{Konfigurationsdatei prüfen:} Bei Bedarf \texttt{config.sample.php} kopieren und anpassen (z.\,B. KI-Provider, Mail-Absender, Upload-Verzeichnis, Standard-Schuljahr).
\end{enumerate}
...
\begin{itemize}
  \item Lehrkräfte- und Klassenverwaltung (inkl. Archivierung)
  \item Schülerverwaltung und Zuordnung zu Klassen
  \item Template-Management (Vorlagen, Felder, Optionslisten, Gruppen, Filterbarkeit)
  \item Globale Einstellungen und Feature-Flags (z.\,B. KI, Elternsicht)
  \item Audit-Log mit Filtern (User, Event, Zeitraum), Pagination, Sortierung und optionaler IP-Anzeige
  \item DSGVO-konformes Löschen personenbezogener Daten
\end{itemize}
...
\begin{itemize}
  ...
  \item KI-Textvorschläge (falls aktiviert) ohne automatische Speicherung
...
\begin{itemize}
  \item Änderungen werden serverseitig gespeichert; Statusanzeigen zeigen \enquote{Speichern}, \enquote{Gespeichert} oder Fehler an.
  \item Live-Vorschau nutzt aktuelle Eingaben und Template-Placeholders.
\end{itemize}
...
\begin{itemize}
  ...
  \item Automatisches Speichern mit Statusanzeige (Speichern/OK/Fehler)
  \item Tokenbasierter Login verhindert Fremdzugriff; Lehrer können Tokens neu generieren
  \item Fortschrittsanzeige pro Schritt und Gesamtreport
  \item TTS-Bar mit Play-/Stop-Button; Stimme und Geschwindigkeit konfigurierbar
\end{itemize}
...
\begin{itemize}
  ...
  \item Bestätigte Abgabe wird im Audit-Log vermerkt
\end{itemize}
...
\begin{itemize}
  ...
  \item Platzhalter wie \texttt{{\{\}student.firstname\}}, \texttt{{\{\}class.label\}} und formatierbare Systemfelder
  \item Einheitliche Export-API unter \texttt{/shared/export\_*}
  \item Zugriffskontrolle: Lehrkräfte nur auf eigene Klassen, Admin auf alle Daten
