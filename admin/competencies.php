...
function fetch_tree(PDO $pdo): array {
  ...
  $comps=$pdo->query("SELECT id,category_id,subcategory_id,code,text_de,text_en,COALESCE(display_type,'rated') AS display_type,is_required,sort_order FROM competencies ORDER BY category_id, COALESCE(subcategory_id,0), sort_order,id")->fetchAll(PDO::FETCH_ASSOC)?:[];
  ...
  foreach($cats as $c){
    ...
    foreach(($compsNoSubByCat[$catId]??[]) as $it){
      $cid=(int)$it['id'];
      $items[]=['id'=>$cid,'type'=>'competency','title'=>(string)$it['code'].' — '.(string)$it['text_de'],'code'=>(string)$it['code'],'text_de'=>(string)$it['text_de'],'text_en'=>(string)$it['text_en'],'display_type'=>(string)($it['display_type']??'rated'),'is_required'=>(int)$it['is_required'],'grades'=>$grades[$cid]??[],'category_id'=>(int)$it['category_id'],'sort_order'=>(int)$it['sort_order']];
    }
...
if(isset($_GET['download_csv_template'])){ $csv="category_de;category_en;subcategory_de;subcategory_en;code;text_de;text_en;display_type;required;grades\n"; $csv.="Sozialkompetenz;Social skills;Kommunikation;Communication;SOZI-001;Hört anderen aufmerksam zu.;Listens attentively to others.;rated;1;1,2\n"; header('Content-Type:text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename="kompetenzen_vorlage.csv"'); echo $csv; exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST' && !isset($_SERVER['HTTP_X_REQUESTED_WITH']) && (string)($_POST['action']??'')==='import_csv'){
  try{ csrf_verify();
    ...
    $line=0; while(($row=fgetcsv($fh,0,';'))!==false){ $line++; if($line===1) continue; if(count($row)<9){ $stats['rows_ignored']++; $stats['errors'][]=$line; continue; }
      if(count($row)>=10){ [$catDe,$catEn,$subDe,$subEn,$code,$textDe,$textEn,$displayType,$required,$grades]=$row; } else { [$catDe,$catEn,$subDe,$subEn,$code,$textDe,$textEn,$required,$grades]=$row; $displayType='rated'; }
      ...
      $displayType=trim(strtolower((string)$displayType)); if(!in_array($displayType,['rated','info'],true)) $displayType='rated';
      $isReq=(trim(strtolower((string)$required))==='1'||trim(strtolower((string)$required))==='ja')?1:0;
      if($compId>0){ $pdo->prepare("UPDATE competencies SET category_id=?,subcategory_id=?,text_de=?,text_en=?,display_type=?,is_required=? WHERE id=?")->execute([$catId,$subId>0?$subId:null,$textDe,$textEn,$displayType,$isReq,$compId]); $stats['competencies_updated']++; }
      else { $so=$subId>0?max_sort($pdo,'competencies','subcategory_id=?',[$subId])+1:max_sort($pdo,'competencies','category_id=? AND subcategory_id IS NULL',[$catId])+1; $pdo->prepare("INSERT INTO competencies(category_id,subcategory_id,code,text_de,text_en,display_type,is_required,sort_order) VALUES (?,?,?,?,?,?,?,?)")->execute([$catId,$subId>0?$subId:null,$code,$textDe,$textEn,$displayType,$isReq,$so]); $compId=(int)$pdo->lastInsertId(); $stats['competencies_created']++; }
  ...
  try{
    ...
    if($a==='create'){
      ...
      if($type==='category'){
        ...
        $de=trim((string)($_POST['text_de']??'')); $en=trim((string)($_POST['text_en']??'')); if($de===''&&$en==='') bad('Kompetenztext DE oder EN erforderlich');
        $displayType=(string)($_POST['display_type']??'rated'); if(!in_array($displayType,['rated','info'],true)) $displayType='rated';
        ...
        $pdo->prepare("INSERT INTO competencies(category_id,subcategory_id,code,text_de,text_en,display_type,is_required,sort_order) VALUES (?,?,?,?,?,?,?,?)")->execute([$cat,$sub>0?$sub:null,$code,$de,$en,$displayType,isset($_POST['is_required'])?1:0,$so]);
    ...
    if($a==='update'){
      ...
      elseif($type==='competency'){ $code=trim((string)($_POST['code']??'')); $de=trim((string)($_POST['text_de']??'')); $en=trim((string)($_POST['text_en']??'')); if($code===''||($de===''&&$en==='')) bad('Code und Texte erforderlich (DE oder EN).'); $displayType=(string)($_POST['display_type']??'rated'); if(!in_array($displayType,['rated','info'],true)) $displayType='rated'; $pdo->prepare("UPDATE competencies SET code=?,text_de=?,text_en=?,display_type=?,is_required=? WHERE id=?")->execute([$code,$de,$en,$displayType,isset($_POST['is_required'])?1:0,$id]); $pdo->prepare("DELETE FROM competency_grade_levels WHERE competency_id=?")->execute([$id]); foreach((array)($_POST['grades']??[]) as $g){$gi=(int)$g;if($gi>=1&&$gi<=4)$pdo->prepare("INSERT INTO competency_grade_levels(competency_id,grade_level) VALUES (?,?)")->execute([$id,$gi]);} }
      else bad('Ungültiger Typ');
      json_out(['ok'=>true,'tree'=>fetch_tree($pdo)]);
    }
  ...
  try { csrf_verify();
    ...
    if($decision==='approve'){
      ...
      if($already<=0){
        ...
        $pdo->prepare("INSERT INTO competencies(category_id,subcategory_id,code,text_de,text_en,display_type,is_required,sort_order) VALUES (?,?,?,?,?,?,?,?)")
          ->execute([$catId,$subId>0?$subId:null,$code,(string)$req['proposal_text_de'],(string)($req['proposal_text_en']??''),'rated',$isRequired,$sort]);
...
function displayTypeSelect(selected='rated'){return `<div class="form-group"><label class="form-label">Darstellung</label><select class="input" name="display_type"><option value="rated" ${selected==='rated'?'selected':''}>Normale Kompetenz mit Bewertung</option><option value="info" ${selected==='info'?'selected':''}>Infozeile ohne Bewertung</option></select></div>`;}
...
function renderComp(k,subId){const el=document.createElement('div'); el.className='tree-node draggable'; el.draggable=true; el.dataset.type='competency'; el.dataset.itemType='competency'; el.dataset.itemId=String(k.id); el.dataset.id=String(k.id); el.dataset.parent=String(subId);const grade=(k.grades||[]).map(g=>`<span class="chip grade-chip grade-${Number(g)||0}">${g}</span>`).join('');const req=k.is_required?'<span class="req-chip required" title="Pflicht">🔒</span>':'<span class="req-chip optional" title="Optional">★</span>';const displayType=(k.display_type||'rated');const infoBadge=displayType==='info'?'<span class="chip" title="Ohne Bewertung">Infozeile</span>':''; el.innerHTML=`<div class="node-head"><span><div class="comp-main">${escapeHtml(k.code)} — ${previewCompetencyText(k.text_de)}</div>${k.text_en?`<div class="comp-sub">${previewCompetencyText(k.text_en)}</div>`:''}<div>${infoBadge}${req}${grade}</div></span><span class="node-actions"><button data-act="edit" data-type="competency" data-id="${k.id}">✏️</button><button data-act="del" data-type="competency" data-id="${k.id}">🗑️</button></span></div>`; return el;}
function escapeHtml(s){return (s??'').toString().replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));}
function previewCompetencyText(s){return escapeHtml((s??'').toString()).replace(/\[checkbox(?:\:[^\]]*)?\]/gi,(m)=>/\btype\s*=\s*radio\b/i.test(m)?'<span class="inline-checkbox-placeholder" title="Optionsfeld" aria-label="Optionsfeld"></span>':'<span class="inline-checkbox-placeholder" title="[checkbox]" aria-label="Checkbox-Platzhalter"></span>').replace(/\[vline\]/gi,'<span class="inline-vline-placeholder" title="[vline]" aria-label="Vertikale Trennlinie"></span>').replace(/\[space\s*=\s*[^\]]+\]/gi,'<span class="inline-space-placeholder" title="[space]" aria-label="Abstand"></span>');}
