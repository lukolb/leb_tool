...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($a === 'add_competency') {
      ...
      if ($subcategoryId > 0) {
        $st = $pdo->prepare("SELECT category_id FROM competency_subcategories WHERE id=? LIMIT 1");
      ...
      $pdo->prepare("INSERT INTO competencies(category_id, subcategory_id, code,text_de,text_en,is_required,sort_order) VALUES (?,?,?,?,?,?,?)")
        ->execute([$categoryId, $subcategoryId > 0 ? $subcategoryId : null, trim((string)$_POST['code']), trim((string)$_POST['text_de']), trim((string)$_POST['text_en']), isset($_POST['is_required'])?1:0, (int)($_POST['sort_order'] ?? 0)]);
    ...
    if ($a === 'csv_import') {
      ...
      while (($r = fgetcsv($fh, 0, ';')) !== false) {
        ...
        $pdo->prepare("INSERT INTO competencies(category_id, subcategory_id, code,text_de,text_en,is_required,sort_order) VALUES (?,?,?,?,?,?,0)")->execute([$catId,$subId,trim((string)$code),$de,$en,(trim($required)==='1'||strtolower(trim($required))==='ja')?1:0]);
        $cid = (int)$pdo->lastInsertId();
...
$compsBySub = [];
foreach ($comps as $c) { $key=(int)($c['subcategory_id'] ?? 0); $compsBySub[$key][] = $c; }
  ...
  <div id="compTree">
    <?php foreach ($cats as $cat): $catId=(int)$cat['id']; ?>
      <details open style="margin-bottom:10px;">
        ...
        <?php $direct = ($compsBySub[0] ?? []); if (!empty($direct)): ?>
          <details style="margin:8px 0 8px 18px;" open>
            <summary><em>Ohne Unterkategorie</em></summary>
            <table class="table" style="margin-left:18px;">
              <tr><th>Code</th><th>DE</th><th>EN</th><th>Typ</th><th>Stufen</th><th>Aktion</th></tr>
              <?php foreach ($direct as $c): if ((int)($c['category_id'] ?? 0)!==$catId) continue; $id=(int)$c['id']; ?>
                <tr draggable="true" data-id="<?=$id?>" data-sort="<?= (int)$c['sort_order'] ?>">
                  <td><?=h((string)$c['code'])?></td><td><?=h((string)$c['text_de'])?></td><td><?=h((string)$c['text_en'])?></td><td><?= ((int)$c['is_required']===1)?'verpflichtend':'optional' ?></td><td><?=h(implode(',', $gradesByComp[$id] ?? []))?></td><td><form method="post" onsubmit="return confirm('Löschen?')"><input type="hidden" name="csrf_token" value="<?=h(csrf_token())?>"><input type="hidden" name="action" value="delete_competency"><input type="hidden" name="id" value="<?=$id?>"><button class="btn">Löschen</button></form></td>
                </tr>
              <?php endforeach; ?>
            </table>
          </details>
...
<details class="card"><summary><strong>Kompetenz hinzufügen</strong></summary><form method="post"><input type="hidden" name="csrf_token" value="<?=h(csrf_token())?>"><input type="hidden" name="action" value="add_competency"><select class="input" name="category_id"><?php foreach($cats as $c):?><option value="<?=$c['id']?>"><?=h($c['name_de'])?> / <?=h($c['name_en'])?></option><?php endforeach;?></select><select class="input" name="subcategory_id"><option value="0">Ohne Unterkategorie</option><?php foreach($subs as $s):?><option value="<?=$s['id']?>"><?=h($s['cat_de'])?> → <?=h($s['name_de'])?></option><?php endforeach;?></select><input class="input" name="code" placeholder="Code"><textarea class="input" name="text_de" required></textarea><textarea class="input" name="text_en" required></textarea><label><input type="checkbox" name="is_required" value="1"> verpflichtend</label><input class="input" name="sort_order" type="number" value="0"><div><?php foreach($gradeOptions as $g):?><label style="margin-right:8px;"><input type="checkbox" name="grades[]" value="<?=$g?>"> <?=$g?></label><?php endforeach;?></div><button class="btn">Speichern</button></form></details>
