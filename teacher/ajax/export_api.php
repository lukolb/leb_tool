...
/**
 * Loads students and (optional) report_instance status in one go.
 */
function load_students_for_export(PDO $pdo, int $classId, int $templateId, string $schoolYear, ?int $onlyStudentId, bool $onlySubmitted): array {
  $whereStudent = '';
  if ($onlyStudentId && $onlyStudentId > 0) {
    $whereStudent = " AND s.id=? ";
  }
  ...
  // Note: params order must match placeholders
  // Placeholders: templateId, schoolYear, classId, [studentId]
  $stmt = $pdo->prepare($sql);

  // reorder params: templateId, schoolYear, classId, optional studentId
...
function compute_export_payload(PDO $pdo, int $classId, int $userId, ?int $onlyStudentId, bool $includeValues, bool $onlySubmitted): array {
  $u = current_user();
...
try {
  ...
  if ($action === 'preview') {
    $payload = compute_export_payload($pdo, $classId, $userId, null, true, $onlySubmitted);
    json_out(['ok'=>true] + $payload);
  }

  if ($action === 'data') {
    $studentId = isset($data['student_id']) ? (int)$data['student_id'] : null;
    $payload = compute_export_payload($pdo, $classId, $userId, $studentId && $studentId>0 ? $studentId : null, true, $onlySubmitted);
    json_out(['ok'=>true] + $payload);
  }
